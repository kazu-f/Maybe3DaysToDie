...
void TerrainSave::Save()
{
	...
	for (int chunk_x = 1; chunk_x < MAX_CHUNK_SIDE; chunk_x++)
	{
		for (int chunk_y = 1; chunk_y < MAX_CHUNK_SIDE; chunk_y++)
		{
			...
			m_SaveDataFile->m_ChunkData[chunk_x][chunk_y].ChunkID[1] = chunk_y;
			for (int terrain_x = 0; terrain_x < ChunkWidth; terrain_x++)
			{
				for (int terrain_y = 0; terrain_y < ChunkHeight; terrain_y++)
				{
					for (int terrain_z = 0; terrain_z < ChunkWidth; terrain_z++)
					{
						...
						//ブロックのIDを記憶
						m_SaveDataFile->m_ChunkData[chunk_x][chunk_y].TerrainID[terrainID[0]][terrainID[1]][terrainID[2]] = TerrainParam.BlockID;
						//ブロックの耐久値を記憶
						m_SaveDataFile->m_ChunkData[chunk_x][chunk_y].TerrainDurable[terrainID[0]][terrainID[1]][terrainID[2]] = TerrainParam.Durable;
						//オブジェクトのタイプを記憶
						m_SaveDataFile->m_ChunkData[chunk_x][chunk_y].TerrainType = TerrainParam.Type;
					}
				}
			}
		}
